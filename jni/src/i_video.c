...
extern SDL_Texture *buttonfire;
extern SDL_Rect rectfire;
...
dboolean touched_screen = false;
dboolean motion_screen = false;
//float touch_x = 0;
//float touch_y = 0;
//float touch_dx = 0;
//float touch_dy = 0;
        ...
        switch (Event->type)
        {
            ...
            case SDL_FINGERMOTION:
                ...
                if (!menuactive) {
                    ...
                    touch_dx[index] = tfe.dx;
                    touch_dy[index] = tfe.dy;
                    D_PostEvent(&event);
                }
                break;
            ...
            case SDL_FINGERUP:
                ...
                if (menuactive) {
                    tfe = Event->tfinger;
                    touch_dx[0] = tfe.x - touch_x[0];
                    touch_dy[0] = tfe.y - touch_y[0];
                } else {
                    int i;
                    for (i=0; i < 2; ++i) {
                        touch_dx[i] = 0;
...
static void SetVideoMode(dboolean output)
{
    ...
    {
        if (output)
            SDL_LogWarn(SDL_LOG_CATEGORY_VIDEO, "Unable to find display %i.", vid_display);
    ...
    if (output)
    {
        ...
        if (displayname[0])
            SDL_Log("Using display %i of %i called \"%s\".", displayindex + 1, numdisplays,
                displayname);
        else
            SDL_Log("Using display %i of %i.", displayindex + 1, numdisplays);
    }
    ...
    if (vid_fullscreen)
    {
        if (!screenwidth && !screenheight)
        {
            ...
            if (output)
                SDL_Log("Staying at the desktop resolution of %ix%i%s%s%s with a %s aspect "
    ...
    else
    {
        ...
        if (!windowx && !windowy)
        {
            ...
            if (output)
                SDL_Log("Created a resizable window with dimensions %ix%i centered on the "
                    "screen.", width, height);
        }
        else
        {
            window = SDL_CreateWindow(PACKAGE_NAME, windowx, windowy, width, height,
                (SDL_WINDOW_RESIZABLE | SDL_WINDOW_OPENGL));
            if (output)
                SDL_Log("Created a resizable window with dimensions %ix%i at (%i,%i).", width,
    ...
    if (output)
    {
        ...
        if (!strcasecmp(rendererinfo.name, vid_scaledriver_direct3d))
            SDL_Log("The screen is rendered using hardware acceleration with the Direct3D 9 "
                "API.");
        else if (!strcasecmp(rendererinfo.name, vid_scaledriver_opengl))
            SDL_Log("The screen is rendered using hardware acceleration with the OpenGL API.");
        else if (!strcasecmp(rendererinfo.name, vid_scaledriver_software))
            SDL_Log("The screen is rendered in software.");

        if (nearestlinear)
        {
            SDL_Log("The %ix%i screen is scaled up to %ix%i using nearest-neighbor "
                "interpolation,", SCREENWIDTH, SCREENHEIGHT, upscaledwidth * SCREENWIDTH,
                upscaledheight * SCREENHEIGHT);
            SDL_Log("    and then down to %ix%i using linear filtering.", height * 4 / 3, height);
        }
        else if (!strcasecmp(vid_scalefilter, vid_scalefilter_linear))
            SDL_Log("The %ix%i screen is scaled up to %ix%i using linear filtering.", SCREENWIDTH,
        ...
        if (vid_capfps)
            SDL_Log("The framerate is capped at %i FPS.", TICRATE);
        else
        {
            if (rendererinfo.flags & SDL_RENDERER_PRESENTVSYNC)
            {
                ...
                SDL_Log("The framerate is capped at the display's refresh rate of %iHz.",
            ...
            else
            {
                if (vid_vsync)
                {
                    if (!strcasecmp(rendererinfo.name, "software"))
                        SDL_LogWarn(SDL_LOG_CATEGORY_VIDEO, "Vertical synchronization can't be enabled in software.");
                    else
                        SDL_LogWarn(SDL_LOG_CATEGORY_VIDEO, "Vertical synchronization can't be enabled.");
                }
                SDL_Log("The framerate is uncapped.");
            }
        }

        SDL_Log("Using the 256-color palette from the PLAYPAL lump in %s file %s.",
            (playpalwad->type == IWAD ? "IWAD" : "PWAD"), uppercase(playpalwad->path));

        if (gammaindex == 10)
            SDL_Log("Gamma correction is off.");
        else
        {
