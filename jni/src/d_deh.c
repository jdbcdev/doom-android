...
{
    return (!fp->lump ? rweof(fp->f) : !*fp->inp || fp->size <= 0);
    //return (!fp->lump ? feof(fp->f) : !*fp->inp || fp->size <= 0);
}

int dehfgetc(DEHFILE *fp)
{
    return (!fp->lump ? rwgetc(fp->f) : fp->size > 0 ? fp->size--, *fp->inp++ : EOF);
...
dboolean CheckPackageWADVersion(void)
{
    ...
    for (i = 0; i < numlumps; ++i)
        ...
        I_Error("lumpinfo[i] %s", lumpinfo[i]->name);
