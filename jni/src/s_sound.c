...
static void InitSfxModule(void)
{
    if (I_SDL_InitSound())
    {
        SDL_Log("SFX playing at a sample rate of %.1fkHz on %i channels.",
            snd_samplerate / 1000.0f, numChannels);
        return;
    }

    SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "The initialization of SFX failed.");
}
...
static void InitMusicModule(void)
{
    if (I_SDL_InitMusic())
    {
        SDL_Log("Using General MIDI for music.");

        CheckTimidityConfig();
        return;
    }

    SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "The initialization of music failed.");
}
...
{
    SDL_Log("Initialize sounds sfxvol: %d, musicvol: %d", sfxvol, musicvol);

    if (M_CheckParm("-nosound") > 0)
    {
        SDL_Log("Found -NOSOUND parameter on command-line. Music and SFX have been disabled.");
        nosound = true;
        nomusic = true;
        nosfx = true;
    }
    ...
    {
        SDL_Log("Found -NOMUSIC parameter on command-line. Music has been disabled.");
        nomusic = true;
    }
    if (M_CheckParm("-nosfx") > 0)
    {
        SDL_Log("Found -NOSFX parameter on command-line. SFX have been disabled.");
        nosfx = true;
    }
...
void S_ChangeMusic(int musicnum, int looping, int cheating)
{
    ...
    if (!handle)
    {
        SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "D_%s music lump can't be played.", uppercase(music->name));
        return;
    }
