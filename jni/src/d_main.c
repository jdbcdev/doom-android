...
#if defined(ANDROID)
    char *packageconfig = M_StringJoin(SDL_AndroidGetInternalStoragePath(), DIR_SEPARATOR_S, PACKAGE_CONFIG, NULL);
#else
    char *packageconfig = M_StringJoin(M_GetExecutableFolder(), DIR_SEPARATOR_S, PACKAGE_CONFIG, NULL);
#endif
    SDL_Log("D_DoomMainSetup packageconfig %s", packageconfig);
#if defined(ANDROID)
    const char *wadfolder = "wad"; //Assets wad
#else
...
#endif
    SDL_Log("D_DoomMainSetup iwadfile %s", iwadfile);
    C_PrintSDLVersions();
    ...
    */
    ...
    if (p)
    {
        ...
        if (gamemode == commercial && temp <= (nerve ? 2 : 1))
        {
            ...
            M_snprintf(lumpname, sizeof(lumpname), "MAP%02i", startmap);
            autostart = true;
            SDL_Log("Found -EXPANSION parameter on command-line. Expansion is now \"%s\".",
    ...
    if (p)
        SDL_Log("Found -WARP parameter on command-line.");
    else
    {
        p = M_CheckParmWithArgs("+map", 1, 1);
        if (p)
            SDL_Log("Found +MAP parameter on command-line.");
    }
    if (p)
    {
        if (gamemode == commercial)
    ...
    SDL_Log("Startup took %02i:%02i:%02i.%i to complete.",
    ...
    if (*startup1 || *startup2 || *startup3 || *startup4 || *startup5)
    {
        C_AddConsoleDivider();
        if (*startup1)
            SDL_Log(removenewlines(startup1));
        if (*startup2)
            SDL_Log(removenewlines(startup2));
        if (*startup3)
            SDL_Log(removenewlines(startup3));
        if (*startup4)
            SDL_Log(removenewlines(startup4));
        if (*startup5)
            SDL_Log(removenewlines(startup5));
    }
}
