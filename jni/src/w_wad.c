...
========================================================================
                               ...
                               DOOM Retro
         The classic, refined DOOM source port. For Android and Linux.
...
wad_file_t *W_AddFile(char *filename, dboolean automatic)
{
    ...
    else
    {
        ...
        //I_Error("Hasta aqui hemos llegado:");
    }
...
dboolean IsFreedoom(const char *iwadname)
{
    ...
    SDL_RWops *fstream = SDL_RWFromFile(iwadname, "rb");
    filelump_t  lump;
    wadinfo_t   header;
    ...
    // read IWAD header
    //if (fread(&header, 1, sizeof(header), fstream) == sizeof(header))
    if (SDL_RWread(fstream, &header, 1, sizeof(header)) == sizeof(header))
    {
        //fseek(fstream, LONG(header.infotableofs), SEEK_SET);
        SDL_RWseek(fstream, LONG(header.infotableofs), RW_SEEK_SET);

        // Determine game mode from levels present
        // Must be a full set for whichever mode is present
        for (header.numlumps = LONG(header.numlumps);
            //header.numlumps && fread(&lump, sizeof(lump), 1, fstream); header.numlumps--)
            header.numlumps && SDL_RWseek(fstream, &lump, sizeof(lump)); header.numlumps--)
        {
...
dboolean HasDehackedLump(const char *pwadname)
{
    ...
    SDL_RWops *fp = SDL_RWFromFile(pwadname, "rb");
    filelump_t  lump;
    wadinfo_t   header;
    const char  *n = lump.name;
    int         result = false;
    ...
    if (SDL_RWread(fp, &header, sizeof(header), 1) == sizeof(header))
    {
        //fseek(fp, LONG(header.infotableofs), SEEK_SET);
        SDL_RWseek(fp, LONG(header.infotableofs), RW_SEEK_SET);

        // Determine game mode from levels present
        // Must be a full set for whichever mode is present
        for (header.numlumps = LONG(header.numlumps);
            //header.numlumps && fread(&lump, sizeof(lump), 1, fp); header.numlumps--)
            header.numlumps && SDL_RWread(fp, &lump, sizeof(lump), 1); header.numlumps--)
        {
