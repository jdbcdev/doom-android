...
void CheckTimidityConfig(void)
{
    if (s_timiditycfgpath[0])
        if (M_FileExists(s_timiditycfgpath))
            SDL_Log("Using TiMidity configuration file %s.", uppercase(s_timiditycfgpath));
        else
            SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "Can't find TiMidity configuration file %s.", uppercase(s_timiditycfgpath));
}
...
dboolean I_SDL_InitMusic(void)
{
    ...
    else
    {
        ...
        else if (Mix_OpenAudio(11025, MIX_DEFAULT_FORMAT, 1, 1024) < 0)
        {
            SDL_QuitSubSystem(SDL_INIT_AUDIO);
            SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "Error initializing SDL_mixer: %s", Mix_GetError());
        }
        else
        {
            SDL_PauseAudio(0);
...
{
    SDL_Log("I_SDL_PlaySong %d", looping);
