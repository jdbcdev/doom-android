...
dboolean I_SDL_InitSound(void)
{
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) < 0) {
        SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "Problem with SDL_INIT_AUDIO");
        return false;
    }
    ...
    /*
    {
        ...
        if (linked->patch != MIX_PATCHLEVEL)
            SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, "The wrong version of SDL2_MIXER.DLL was found. "PACKAGE_NAME" requires "
                "v%d.%d.%d, not v%d.%d.%d.", linked->major, linked->minor, linked->patch,
                MIX_MAJOR_VERSION, MIX_MINOR_VERSION, MIX_PATCHLEVEL);
    }
    */
    ...
    {
        SDL_LogWarn(SDL_LOG_CATEGORY_AUDIO, Mix_GetError());
        return false;
    }
